...
const USER_AGENT = "deepseek-harness/dsh-web-search-litellm/0.1.0";
...
class LiteLLMResponsesProvider {
	...
	async search(request, signal) {
		...
		if (signal?.aborted === true) throw this.fail("litellm web search aborted", "WEB_ABORTED", signal.reason);
		const timeoutSignal = options.timeoutMs !== void 0 && options.timeoutMs !== null ? AbortSignal.timeout(options.timeoutMs) : void 0;
		const effectiveSignal = timeoutSignal !== void 0 && signal !== void 0 ? AbortSignal.any([signal, timeoutSignal]) : timeoutSignal ?? signal;
		const body = {
			model: options.model,
		...
		try {
			...
			if (timeoutSignal?.aborted === true) throw this.fail(`litellm web search timed out after ${options.timeoutMs}ms`, "WEB_TIMEOUT", error);
		...
		if (signal?.aborted === true) throw this.fail("litellm web search aborted", "WEB_ABORTED", signal.reason);
		if (timeoutSignal?.aborted === true) throw this.fail(`litellm web search timed out after ${options.timeoutMs}ms`, "WEB_TIMEOUT");
